...
def print_Visiblerow(s,locations,locationObs):
    k = 0
    while (k+1<5):
        f = open(text,'a')
        if (locations[k+1] not in locationObs):
            txt.write(''.join([' ','(',s,' loc'+str(locations[k]),' ','loc'+str(locations[k+1]),')']))
        if (locations[k] not in locationObs):
            txt.write(''.join([' ','(',s,' loc'+str(locations[k+1]),' ','loc'+str(locations[k]),')']))
        k+=1
        f.close()
    write_new_line()

def print_Visiblecol(s,locations,locationObs):
    k = 0
    while (k+1<4):
        f = open(text,'a')
        if(locations[k+1] not in locationObs):
